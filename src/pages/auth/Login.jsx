...
const LoginPage = () => {
  ...
  const handleReset = async (e) => {
      ...
      if (error) {
          ...
          setSuccess("Recovery Email successfully dispatched! Validate your inbox for the reset link.");
  ...
  const handleLogin = async (e) => {
    ...
    if (userRole === 'blocked') {
        setError('Your account has been blocked or banned by an administrator.');
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-8">
        ...
        <div className="flex gap-4 mb-6">
          <button 
            ...
            disabled={loading}
            className="flex-1 flex items-center justify-center gap-2 py-2.5 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50 transition disabled:opacity-70 disabled:cursor-not-allowed"
          >
            {/* Facebook Logo SVG */}
        ...
        {/* Login/Reset Form */}
        <form className="space-y-4" onSubmit={isResetting ? handleReset : handleLogin}>
          {success && (
              <div className="bg-green-50 text-green-700 p-3 mb-2 rounded-md text-sm border border-green-200 mt-[-10px]">
                  {success}
              </div>
          )}
          {error && (
